...
- `cmd/lib/process.sh` 只依据本应用 PID 文件停止进程，不扫描或杀死其他用户进程；默认服务端口为 8649。
...
Manager 通过 Unix socket 提供本地 API，并由 `app/ui/config` 注册只读桌面入口。界面只有“总览”“Agents”“设置”三个标签；总览同时展示 HStudio、Hermes Studio Runtime 状态及启动、停止、重启和更新操作。顶部的“打开 Hermes Studio”按钮会在新标签页打开 fnOS 为 `HStudio` 服务分配的独立入口（生产环境形如 `https://hstudio.<主机>/`）；桌面图标也指向该页面。管理员操作要求 fnOS 网关注入的 `X-Trim-Userid` 与 `X-Trim-Isadmin` 请求头；日志接口会脱敏 token、cookie、password 等敏感字段。

设置页的 FPK 更新功能当前只读取 `leen2175/fnos-HStudio` 的最新 GitHub Release、比较版本并提供发布页信息，与 Studio Runtime 更新相互独立；它尚不校验 Release 中是否存在兼容的 FPK，也不会下载或安装 FPK，因此不能更新 Manager 自身。首个 FPK Release 发布前会显示尚无可用版本。完整 FPK 升级仍需通过 fnOS 应用中心手动安装，开发或重复测试也可使用 `appcenter-cli install-fpk`。

总览中的 Hermes Studio 状态卡片只在需要时显示安装、重试或确有新版本的更新；启动、停止、重启与更新都会按后台任务持续反馈结果。更新前比较严格 SemVer，禁止 npm latest 低于当前 Runtime 时降级。更新会先在隔离目录取得并校验精确版本，停止 Studio 后才原子切换；带校验和的阶段 journal 可在 Manager 被终止或断电后自动回滚，损坏 journal 则保留并 fail-closed。成功后重建 Skill 合并源，大目录旧包在正确性提交后延迟清理。

`Agents` 页分别管理 Hermes Agent 与 Coding Agents。Hermes Agent 首次安装或从旧 PyPI 环境迁移时，按当前 Hermes Studio release 的 Runtime 清单拉取固定 tag/commit，在隔离目录用带哈希依赖清单完成 editable 安装和验证，然后切换到应用私有工作树；本地分支保持为 `main`，后续 Manager 更新直接执行官方 `hermes update`。Hermes Studio 启动时会把 Agent Bridge 固定到 HStudio 私有 venv，并使用 `TRIM_PKGVAR` 下的 Unix socket（缺失时回退到 `data/run`）。该流程复用 fnOS Python/Node.js，不执行远程安装脚本，也不覆盖 `$HERMES_HOME` 配置。Codex（`@openai/codex`）、Claude Code（`@anthropic-ai/claude-code`）和 Pi（`@earendil-works/pi-coding-agent` 最新版）统一写入应用用户的 `.npm-global`，Pi 同时安装 npm 最新版 `pi-mcp-adapter`。
...
Lite FPK 安装阶段只安装适配层并启动 Manager。首次 bootstrap 优先断点下载并校验官方 GitHub Runtime archive；GitHub 不可达时，可在隔离临时前缀从所选 npm 镜像取得固定版本，再经相同健康检查发布为 bundled Runtime，绝不写坏用户 `.npm-global`。Offline 构建不会自行下载 Runtime，必须提供与 `config/runtime-manifest.json` 完全匹配的 archive；构建器会联网确认它仍是 GitHub latest 正式版本，再将其内嵌。Hermes Agent 源码和 Python wheels 仍需首次安装时联网获取，两个变体都使用同一份固定版本与依赖哈希。构建器会在 `artifacts/ARTIFACTS.md` 生成当前产物及校验值。

`trim-cli` 不预置账号、密码或 session；首次连接仍需用户显式登录。包内只保留 fnOS/Linux 所需的两个架构文件，并由 `config/runtime-manifest.json` 固定 SHA-256。

## 上游、许可证与发布边界

仓库当前锁定上游 `v0.7.16`、完整提交 `292a675d6cc82b50baae51d1268980186a985929`。官方 Runtime archive 的 SHA-256 为 `457d5745c58d6c5991b729c403bc890d564c48de62703a435a376a1e13f9a797`。Offline 构建会联网复核 latest release，锁定落后时直接失败；归档缺失、尺寸、摘要或布局不匹配也不会生成 FPK。
...
CI 当前检查 Hermes Studio Runtime 与 `trim-cli` 两项 `licenseStatus`；任一项不是 `redistribution-approved` 时仍会构建和验证 Lite/Offline，但不会上传 FPK artifact，工作流的只读权限也不能创建公共 Release。当前 `trim-cli` 已按公共工具批准，Hermes Studio Runtime 仍受该闸门约束。
