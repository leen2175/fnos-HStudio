...
- npm 的 `hermes-web-ui@latest` 在本次核对时仍为 `0.7.15`。Manager 必须先比较版本，禁止用 npm latest 把 `0.7.16` 降级；Offline 构建以清单锁定的 GitHub release 及其配套 JSON/asset 为准。
...
权威上游核对由 Hermes Studio 的完整 commit 锁、官方 release metadata 与 CI 只读 checkout 保证。Offline 构建验证锁定的正式 Web UI release、归档和校验值，不依赖随后可能变化的 `releases/latest`；定时 `upstream-watch` 单独负责报告锁定版本落后。Hermes Studio Runtime 许可未明确批准时，CI 仍阻止上传 Offline 产物，但不会无故阻止不携带 Runtime 的 Lite 检查产物。
