...
#!/bin/bash
if [ "${1:-}" = view ]; then echo 9.8.7; exit 0; fi
...
test -d "$npm_stage_path" # Canonical npm prefix and user data are preserved for retry.
test -f "$T/home/data/runtime/studio/.npm-stage.keep/SENTINEL"
echo 'PASS stop terminates the exact bounded npm install process group'
