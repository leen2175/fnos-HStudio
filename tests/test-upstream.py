...
pkg = up / "package.json"
agent_release_file = up / "packages" / "desktop" / "build" / "runtime-release.json"
...
if not agent_release_file.is_file():
    raise SystemExit("upstream Hermes Agent release metadata missing: " + str(agent_release_file))
upstream = json.loads(pkg.read_text(encoding="utf-8"))
agent_release = json.loads(agent_release_file.read_text(encoding="utf-8"))
...
agent = manifest["hermesAgent"]
source = agent_release.get("hermesSource", {})
if agent.get("source") != f"EKKOLearnAI/hermes-studio@v{upstream['version']}":
    raise SystemExit("Hermes Agent pin source does not match the Studio release")
if agent.get("version") != agent_release.get("hermesAgentVersion"):
    raise SystemExit("Hermes Agent version differs from the Studio runtime release")
for key in ("repository", "ref", "commit"):
    if agent.get(key) != source.get(key):
        raise SystemExit(f"Hermes Agent {key} differs from the Studio runtime release")
