...
payload = BUILDER.create_app_payload(manifest)
with tarfile.open(fileobj=io.BytesIO(payload), mode="r:gz") as archive:
    ...
    for platform, name in expected_bins.items():
        member = members[name]
        assert member.mode & 0o111, f"{name} is not executable"
    ...
    assert wrapper.mode & 0o111, "trim-cli wrapper is not executable"
...
print("PASS online FPK trim-cli payload: Linux x64 + ARM64 only")
